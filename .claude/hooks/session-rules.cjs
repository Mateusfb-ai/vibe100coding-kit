...
const regrasFixas = [
  ...
  '2. SKILL A CADA COMANDO: se há 1% de chance de uma skill ajudar, use (feature → brainstorming + spec · bug → systematic-debugging · revisão antes de integrar → painel de code-review · teste de tela → agent-browser · UI → skill de design · texto para humano → humanizer · entrega pronta → checar-entrega · subir para a main → sync-main · não sabe se existe skill → find-skills). Escada da preguiça (Ponytail) em todo código: pula → reusa → stdlib → dependência → escreve.',
